...
def healthcheck(*, shop_id: str | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        if home.status_code >= 400:
            ...
            if api.status_code == 200 and payload.get("code") == 200:
                result["ok"] = True
                result["message"] = "FastMoss reachable; recentData OK"
                ...
                result["failure_class"] = classify_failure(
                    api.status_code, (api.text or "")[:500]
                )
                result["message"] = (
                    f"FastMoss recentData failed HTTP {api.status_code} "
                    f"api_code={payload.get('code')} "
    ...
    if not result["ok"]:
        ...
        if fc == "auth" or (not cookie_on and fc in {"blocked", "unknown"}):
            result["action"] = (
            ...
            result["action"] = (
                ...
                "Add a browser Cookie via FASTMOSS_COOKIE, slow down requests, and retry later."
            )
        elif fc == "path":
            result["action"] = (
    ...
    level(
        "FastMoss healthcheck ok=%s cookie=%s home=%s detail=%s api=%s class=%s msg=%s",
        result["ok"],
        cookie_on,
