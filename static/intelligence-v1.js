...
(function () {
  ...
  function animateSobBars(root, options = {}) {
  ...
  function renderBusinessTableRow(s, isOpen) {
    ...
    const chevron =
      window.SlaShopDetail?.renderChevron(isOpen) ||
      `<button type="button" class="si-sla-row-toggle" data-sla-row-toggle aria-expanded="${isOpen ? "true" : "false"}" aria-label="Expand shop detail"><svg class="si-sla-row-toggle__icon" viewBox="0 0 24 24" width="16" height="16" aria-hidden="true"><path fill="currentColor" d="M9.29 6.71a1 1 0 0 1 1.42 0L15 11l-4.29 4.29a1 1 0 0 1-1.42-1.42L12.17 11 9.29 8.12a1 1 0 0 1 0-1.41z"/></svg></button>`;
    const tkMom =
  ...
  function renderBusinessTableBodyRows(sellers, expandedSet, detailCache) {
    ...
    return sellers
      .map((s) => {
        ...
        const expand =
          isOpen && detail?.ShopDetailExpandableRow?.(s, true, detailState)
            ? detail.ShopDetailExpandableRow(s, true, detailState)
            : "";
        return main + expand;
      })
      .join("");
  ...
  function bindBusinessPagination(root, st, filteredLength) {
    ...
    pager.querySelector("[data-sla-page-size]")?.addEventListener("change", (ev) => {
      ...
      st.page = 1;
      paintBusinessList({ refreshSummary: false });
    });
  ...
  function paintBusinessList(options = {}) {
    const refreshSummary = options.refreshSummary !== false;
    const el = containers.siBusiness;
    const st = state.business;
    if (!el || !st.raw) return;
    if (refreshSummary) paintBusinessSummarySob();
    ...
    window.SlaShopDetail?.syncExpandedRows?.(
  ...
  function setupBusiness(data) {
    ...
    if (!state.business.shellReady) {
      ...
      const listEl = el.querySelector("[data-si-list]");
      window.SlaShopDetail?.initDelegation?.(
        listEl,
        () => state.business.currentPageRows,
        state.business.expanded,
        state.business.shopDetail
      );
